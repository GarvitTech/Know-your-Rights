...
export default function ProfileSetup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(age) || age < 1 || age > 120) { setError("Please enter a valid age (1–120)"); return; }
    setLoading(true);
    try {
      await api.post("/profile", { ...form, age });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">We'll personalise your rights based on your details</p>
        </div>
        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                {COUNTRIES.map((c) => (
                  <button key={c} type="button"
                    ...
                    <span className="text-lg sm:text-xl">{COUNTRY_FLAGS[c]}</span>{c}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <select className="input" value={form.occupation}
